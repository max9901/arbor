...
namespace arb::EMC_catalogue::kernel_adex_DCN {
...
[[maybe_unused]] auto* _pp_var_I_pc_dcn          = pp->state_vars[1];\
...
[[maybe_unused]] auto* _pp_var_I_pc_max          = pp->parameters[11];\
[[maybe_unused]] auto* _pp_var_tau_pc            = pp->parameters[12];\
//End of IFACEBLOCK
...
// interface methods
    ...
    static void advance_state(arb_mechanism_ppack* pp) {
        ...
        for (unsigned i = 0; i < _pp_var_width; i++) {
            ...
            const arb_value_type grad_i_pc_dcn = ((_pp_var_I_pc_max[i] - _pp_var_I_pc_dcn[i]) )/_pp_var_tau_pc[i];
            ...
            const arb_value_type grad_vm = (il + id + _pp_var_I[i] -_pp_var_w[i] -_pp_var_I_pc_dcn[i] )/_pp_var_C[i];

            _pp_var_w[i] += 1e3 * dt * grad_w;
            _pp_var_I_pc_dcn[i] += 1e3 * dt * grad_i_pc_dcn;
        ...
        for (arb_size_type c = 0; c<ncell; ++c) {
            ...
            for (auto p = begin; p<end; ++p) {
                ...
                if (p->mech_id==_pp_var_mechanism_id) {
                    _pp_var_I_pc_dcn[i_] = _pp_var_I_pc_dcn[i_] + weight;
                }
            }
        }
    }
...
arb_mechanism_interface* make_arb_EMC_catalogue_adex_DCN_interface_multicore() {
    ...
    result.partition_width = arb::EMC_catalogue::kernel_adex_DCN::simd_width_;
    result.backend=arb_backend_kind_cpu;
    result.alignment=1;
    result.init_mechanism  = (arb_mechanism_method)arb::EMC_catalogue::kernel_adex_DCN::init;
    result.compute_currents= (arb_mechanism_method)arb::EMC_catalogue::kernel_adex_DCN::compute_currents;
    result.apply_events    = (arb_mechanism_method_events)arb::EMC_catalogue::kernel_adex_DCN::apply_events;
    result.advance_state   = (arb_mechanism_method)arb::EMC_catalogue::kernel_adex_DCN::advance_state;
    result.write_ions      = (arb_mechanism_method)arb::EMC_catalogue::kernel_adex_DCN::write_ions;
    result.post_event      = (arb_mechanism_method)arb::EMC_catalogue::kernel_adex_DCN::post_event;
    return &result;
}}
