...
auto* _pp_var_I_pc_dcn          __attribute__((unused)) = params_.state_vars[1];\
...
auto* _pp_var_I_pc_max          __attribute__((unused)) = params_.parameters[11];\
auto* _pp_var_tau_pc            __attribute__((unused)) = params_.parameters[12]; \
//End of IFACEBLOCK
        ...
        void mechanism_adex_DCN_gpu_init_(arb_mechanism_ppack* p) {
            unsigned block_dim = 128;
            unsigned grid_dim = ::arb::gpu::impl::block_count(p->width, block_dim);
            init<<<grid_dim, block_dim>>>(*p);
        }
        void mechanism_adex_DCN_gpu_compute_currents_(arb_mechanism_ppack* p) {}
        void mechanism_adex_DCN_gpu_advance_state_(arb_mechanism_ppack* p) {
            unsigned block_dim = 128;
            unsigned grid_dim = ::arb::gpu::impl::block_count(p->width, block_dim);
            advance_state<<<grid_dim, block_dim>>>(*p);
        }
        void mechanism_adex_DCN_gpu_write_ions_(arb_mechanism_ppack* p) {}
        void mechanism_adex_DCN_gpu_post_event_(arb_mechanism_ppack* p) {}
        void mechanism_adex_DCN_gpu_apply_events_(arb_mechanism_ppack* p, arb_deliverable_event_stream* stream_ptr) {
