...
//     resources.gpu_id = arbenv::default_gpu();
    ...
    auto defaultCat = (arb::mechanism_catalogue*)&arb::global_default_catalogue();
    cat->import(*defaultCat,"");      //actually add the catalogue we are interested in.
...
arb::cable_cell IO_cell(){
    ...
    decor.paint("soma"_lab, arb::density{na_s});

    arb::mechanism_desc kdr("kdr");
    decor.paint("soma"_lab, arb::density{kdr});

    arb::mechanism_desc ks("k");
    decor.paint("soma"_lab, arb::density{ks});

    arb::mechanism_desc cal("cal");
    decor.paint("soma"_lab, arb::density{cal});

    //dend mechs
    arb::mechanism_desc cah("cah");
    cah["gmax"] =0.010 * 1.7 / 2;
    decor.paint("dend"_lab, arb::density{cah});

    arb::mechanism_desc kca("kca");
    kca["gmax"] = 0.200 * 0.7 * 1.5;
    decor.paint("dend"_lab, arb::density{kca});

    arb::mechanism_desc h("h");
    h["gmax"] = 0.025 * 1.7;
    decor.paint("dend"_lab, arb::density{h});

    arb::mechanism_desc cacc("cacc");
    cacc["gmax"] = 0.007;
    decor.paint("dend"_lab, arb::density{cacc});

    //axon mechs
    arb::mechanism_desc na_a("na_a");
    decor.paint("axon"_lab, arb::density{na_a});

    arb::mechanism_desc k("k");
    decor.paint("axon"_lab, arb::density{k});

    //global mechs
    arb::mechanism_desc leak("leak");
    leak["gmax"] = 1.3e-05;
    decor.paint("(all)"_reg, arb::density{leak});

    arb::mechanism_desc ou_noise("ou_noise/seed=10");
    decor.paint("(all)"_reg, arb::density{ou_noise});

    // Add gap junction sites at the end of the branch
    decor.place(arb::mlocation{1, 1}, arb::junction{"cx36"},"local_0");
    decor.place(arb::mlocation{3, 1}, arb::junction{"cx36"},"local_1");
    decor.place(arb::mlocation{4, 1}, arb::junction{"cx36"},"local_2");

    // Add a spike detector to the soma at the beginning
//    decor.place(arb::mlocation{0,0}, arb::threshold_detector{10});

    // Attach a stimulus (should not be used :D )
    //    auto stim = arb::i_clamp::box(0, stim_duration, 0.4);
    //    decor.place(arb::mlocation{0, 0.5}, stim);

//    decor.place(arb::mlocation{0, 0.5},arb::synapse("adex")     ,"syn1");
//    decor.place(arb::mlocation{0, 0.5},arb::synapse("adex_PC")  ,"syn2");
//    decor.place(arb::mlocation{0, 0.5},arb::synapse("adex_DCN") ,"syn3");
